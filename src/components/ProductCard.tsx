        ...
        <div className="space-y-6">
          <div className="border-b pb-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><span className="font-medium">Visibility:</span> {product.visibility}</div>
              <div><span className="font-medium">Type ID:</span> {product.type_id}</div>
              <div><span className="font-medium">Created At:</span> {product.created_at}</div>
              <div><span className="font-medium">Updated At:</span> {product.updated_at}</div>
          ...
          <div className="border-b pb-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(product.extension_attributes.stock_item).map(([key, value]) => (
                <div key={key}>
                ...
                {Object.entries(entry).map(([key, value]) => (
                  <div key={key}>
                    <span className="font-medium">{key}:</span> {
                      Array.isArray(value) ? value.join(', ') : value?.toString() || 'null'
                    }
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-4">
              {product.custom_attributes.map((attr, index) => (
                <div key={index} className="border-b pb-4">
                  <div className="font-medium text-blue-600">{attr.attribute_code}</div>
